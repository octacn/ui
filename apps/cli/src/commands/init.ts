...
export const initOptionsSchema = z.object({
  ...
  template: z
    ...
    .refine(
      (val) => {
        if (val) {
          return "string for temp init"
          // TEMPLATES[val as keyof typeof TEMPLATES]
        }
        return true
      },
      {
  ...
  baseColor: z
    ...
    .refine(
      (val) => {
        if (val) {
          return "string for temp init"
          //  BASE_COLORS.find((color) => color.name === val)
        }

        return true
      },
      {
        message: `Invalid base color. Please use 'neutral', 'gray', 'zinc', 'stone', 'slate'`,
      }
    ),
  baseStyle: z.boolean(),
...
export const init = new Command()
  ...
  .action(async (components, opts) => {
    try {
      logger.log("Loading environment files...")
    } catch (error) {
      logger.break()
      handleError(error)
    } finally {
